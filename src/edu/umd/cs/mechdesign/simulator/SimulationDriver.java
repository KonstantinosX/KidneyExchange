 ...
 * time unit in this simulation corresponds to one week passing.
 * 
 * @author kostasx
 *
 */
public class SimulationDriver {
	...
	public static void main(String[] args) {
		...
		/**
		 * The vertices that got removed from the pool because their transplant
		 * is already scheduled
		 * */
		Set<Vertex> scheduledForTransplant = new HashSet<>();
		...
		while (true) {
			...
			if (matchingsTime >= timeLimit) {
				...
				matchingsTime += matchingsInterval;
			}
		...
		while (currTime < timeLimit) {
			...
			if (currEvent.getType().equals(EventType.CONDUCT_MATCHINGS)) {
				...
				for (Cycle c : s.getMatching()) {
					...
					/*
					 * individual transplants in chains get scheduled
					 * indipendently from eachother
					 */
					if (Cycle.isAChain(c, pool)) {
						/*
						 * reverse the edges so that we iterate over them in the
						...
						for (int i = 0; i < chainEdges.size() - 1; i++) {
							...
							l.add(e);
							Cycle chainPortion = Cycle.makeCycle(l, l.size());
		...
		Utils.serializePatients(patientsByEntryTime, patientsByExitTime,
				patientAges, transplantTimes, patientsPath, timeLimit);
	...
	/**
	 * Chains are stored backwards for some reason... This reverses them so that
	 * they're iterated over correctly.
	 * 
	 * @param chain
	 * @return
	 */
	private static void reverseChain(Cycle chain) {
		Collections.reverse(chain.getEdges());
	}
